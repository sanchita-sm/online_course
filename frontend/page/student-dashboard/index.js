...
async function loadDashboard() {
  try {
    ...
    const progressData = await Promise.all(enrolled.map(async c => {
      ...
      completedLessons += prog.completed_lessons || 0
      return { ...c, progress: prog }
    }))

    document.getElementById('statCompleted').textContent = completedLessons
    ...
    return `
      <a href="../lesson-view/index.html?course_id=${c.id}" class="course-card student-course-card">
        ...
        <div class="course-card-body">
          <div class="course-card-tags"><span class="tag tag-category">${c.category_id}</span></div>
          <h3 class="course-card-title">${c.title}</h3>
          <div class="progress-bar-wrap"><div class="progress-bar-track"><div class="progress-bar-fill" style="width:${pct}%"></div></div><span class="progress-pct">${pct}%</span></div>

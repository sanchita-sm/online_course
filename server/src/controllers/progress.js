...
const markLessonComplete = async (req, res, next) => {
    try {
        ...
        const result = await ProgressModel.markLessonComplete({
...
const getLessonProgress = async (req, res, next) => {
    try {
        ...
        const progress = await ProgressModel.getLessonProgress(
            student_id,
            course_id
        )
        res.json(progress)
...
const getCourseProgress = async (req, res, next) => {
    try {
        const { student_id, course_id } = req.params
        const progress = await ProgressModel.getCourseProgress(

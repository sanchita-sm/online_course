...
function renderProgress(courses) {
  ...
  if (!courses.length) { list.innerHTML = `<div class="empty-state">ยังไม่ได้ลงทะเบียนคอร์สใด <a href="../courses/index.html" class="auth-link">ค้นหาคอร์ส</a></div>`; return }
      ...
      <div class="progress-course-card">
        <div class="progress-course-left">
          ...
          <div class="progress-course-info">
            ...
            <div class="course-card-tags" style="margin-top:4px"><span class="tag tag-category">${c.category_id}</span></div>
          </div>
        </div>
        <div class="progress-course-right">
          <span class="progress-status ${statusClass}">${statusText}</span>
          <div class="progress-bar-wrap" style="width:200px"><div class="progress-bar-track"><div class="progress-bar-fill" style="width:${pct}%"></div></div><span class="progress-pct">${pct}%</span></div>
          <span class="progress-label">${c.done}/${c.total} บทเรียน</span>
          <a href="../lesson-view/index.html?course_id=${c.id}" class="btn-continue">${pct === 100 ? '🔁 ทบทวน' : '▶ เรียนต่อ'}</a>
        </div>
      </div>`
  }).join('')
}
